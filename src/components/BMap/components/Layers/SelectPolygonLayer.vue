...
export default {
  ...
  methods: {
    ...
    loadAreaEdgeLayer(name, level) {
      this.level = level;
      this.$store.commit("setActiveLevel", level);
      let levelDict = { 1: 8.5, 2: 10.5, 3: 11.5, 4: 13 };
      let highDict = { 1: 20000, 2: 5000, 3: 3000, 4: 1000 };
    ...
    loadAreaEdge(path, list, isClear = true) {
      ...
      for (let name of list) {
        ...
        polygon.addEventListener("onmouseover", (e) => {
          ...
          this.sp.setStrokeOpacity(0);
          this.map.addOverlay(this.sp);
          this.sp.addEventListener("onmouseout", (e) => {
            this.map.removeOverlay(this.sp);
          });
          e.cancelBubble = true; //取消当前事件的冒泡
          if (e.stopPropagation) {
            e.stopPropagation();
          }
        });
        polygon.addEventListener("onclick", (e) => {
          console.log(e, 2, name);
          this.map.clearOverlays();
          this.loadAreaEdgeLayer(name, this.level + 1);
          e.cancelBubble = true; //取消当前事件的冒泡
          if (e.stopPropagation) {
            e.stopPropagation();
          }
